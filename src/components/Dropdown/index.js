...
class Dropdown extends Component {
  ...
  static defaultProps = {
    ...
    renderItem: ( {item, highlighted, selected} ) => <li
      className={'b-dropdown__item' + (selected ? ' b-dropdown__item_selected' : '') + (highlighted ? ' b-dropdown__item_highlighted' : '')}>{selected ?
      <i className="fa fa-check"/> : ''}{typeof(item) === 'object' ? item.title : item}</li>,
  ...
  render() {
    ...
    const renderedItems = this.items.map( ( item, index ) => {
      ...
      const selected    = this.props.value === (typeof(item) ==='object' ? item.value : item);
      return React.cloneElement( Item( {item, index, highlighted, selected} ), {
        onMouseEnter: this.handleMouseEnter( index ),
        onMouseDown: this.handleMouseDown,
        onClick: this.handleSelectItem( item ),
        key: typeof(item) === 'object' ? `${index}_temp_key` : `${index}${item.substr( 0, 3 )}`
      } );
    } );
